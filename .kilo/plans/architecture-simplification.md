...
### Files to change
- `MonoGameLearning.Game/Entities/Enemy/EnemyEntity.cs` — remove `= null` default, remove null guards
- `MonoGameLearning.Game/Levels/EnemyPool.cs` — `DefaultFactory` already passes director, no change needed
- `MonoGameLearning.Game.Tests/EnemyPoolTests.cs` — `MockFactory` + `TestEnemyPool` may need adjustment
- `MonoGameLearning.Game.Tests/LevelDirectorTests.cs` — `TestEnemyPool` may need adjustment
...
### Files to change
- Move `MonoGameLearning.Core/Combat/OilDrumBehavior.cs` → `MonoGameLearning.Game/Entities/Props/OilDrumBehavior.cs`
