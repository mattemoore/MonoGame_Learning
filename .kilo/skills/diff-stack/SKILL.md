---
name: diff-stack
description: Split a large unstaged working-tree change set into a sequence of stacked PRs using the official `gh stack` GitHub CLI extension. The assistant analyzes the current unstaged changes, builds careful local commits formingthe branch chain,then prompts the user to run the exact `gh stack init` and `gh stack submit` commands to track the stack locally,and publish it to GitHub - without the assistant ever pushing tothe remote.
---

Split large multi-concern unstaged working-tree changes into an ordered stack of small PRs that can be reviewed and merged one after another, usingthe official [`gh stack`](https://github.com/github/gh-stack)extension. The assistant owns everything local:it inspects the unstaged changes, groups them into logical PR-sized layers, builds the local commit chain branch-by-branch,and verifies every snapshot compiles and tests green. Remote bookkeeping (pushing branches, creating one PR per branch with the correct base,and linking them as a Stack on GitHub)is done by the official extension's `gh stack init` + `gh stack submit` commands. After the assistant finishes buildingthe chain,it hands the user precisely those commands as copy-paste prompts - the assistant never runs `gh stack submit`, `gh stack push`, `gh stack init`, or `git push`.
...
- The unstaged working tree has many modified/untracked files(roughly 10+)spanning multiple independent concerns.
...
- Stack metadata lives in `.git/gh-stack`(local JSON;never committed;sent to the remote only when the user runs a push/submit command)。

## The assistant's workflow

### 1. Analyze the current unstaged changes

- `git status --short` and `git diff --stat HEAD`(plus `git diff HEAD` for content)to see the full change set。
...
- Cluster files by concern(e.g., content/atlas renames, sprite split, combat/animation fix,d level wiring, tests, docs/config)。
...
- Produce a small PR table(branch name → one-line title → base → files included),which you show the user alongside the final commands。
...
- When done,the working tree returns to a clean state:every group has been committed across the chain。

### 4. Prompt the user to track the stack locally:`gh stack init`

- From the trunk(e.g., `main`),the user adopts the branches into a tracked stack,bottom-to-top:

  ```sh
  gh stack init <branch1> <branch2> ... <branchN>
  ```

- `init` adopts existing branches automatically,and creates any missing ones. If the trunk isn't the repo's default branch,pass `--base <trunk>`. Metadata stays local in `.git/gh-stack`,so nothing hits the remote yet。
- After it,running `gh stack view`(or `--short`)lets the user verify the tracked tree:each layer shows its parent - branch1's parent is the trunk,branch2's parent is branch1,and so on。

### 5. Prompt the user to publish the stack:`gh stack submit`

- Present the PR table(branch → title → base → files included)and this exact command:
...
- The user-runnable command handoff step looks like this(after step 3 completes):
  ...
  > ```
  >
  > `init` tracks the local chain;`submit` pushes every branch,and creates one PR per branch(linked as a Stack). Verify with `gh stack view`。

## Keeping the stack in sync(after the stack is live)

- A change to an early layer(e.g., PR #1)does not appear on downstream PRs until the user restacks and resubmits。
...
- Stack metadata is local-only;nothing touches the remote until the user runs `submit`/`push`。
- The assistant never pushes,creates PRs,,or runs `gh stack init`:those commands are for the user to run。

## Definition of done

- Local branch chain exists:branch1 = main + group1,branch2 = branch1 + group2,and so on;;each compiles and passes tests。
- The working tree returns to a clean state(all groups committed across the chain)。
- The user has been prompted with, and will run themselves,(a)`gh stack init <branches...>` to track the chain locally,and(b)`gh stack submit` to push all branches and create the linked PR stack on GitHub。

- User confirmed `gh stack` is installed/authenticated。
