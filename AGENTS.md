...
* **`Weapons`**: Static melee weapon defs (e.g., `BatWeapon`) built from `Core.Combat.MeleeWeaponDef` that swap `Attack1Move`/`AttackMove` while armed. Swing rendering uses a 4-frame `bat-texture.png`/`bat.json` atlas via `AnimatedSprites.BatSprite`, positioned per `attack` frame via `SwingAnchors`/`CarryAnchor`; hitboxes fire at swing apex (frames 2–3) only. The overlay is drawn directly by atlas region name (`MeleeWeaponDef.ResolveWeaponRegionName`: `<SwingPrefix>-<NN>` while attacking, else the single `CarryRegion` hold pose) — it never frame-steps a sprite, so it stays in sync with the player's attack animation with no `SetFrame()`/`TextureRegion` workaround. `AnimatedSprites.BatPickupSprite` owns the separate static `bat-pickup.png` texture used by `WeaponPickupEntity` for the dropped-pickup icon (same pattern as `FoodPickupSprite`/`apple-pickup.png`). Source art is authored in **Aseprite** under `MonoGameLearning.Game/Sources/Weapons/<Name>/` and exported as a sprite-sheet texture + JSON (Aseprite's `JSON Array` or `JSON Hash` data format); `Utils/aseprite_to_monogame_extended.py` converts that export to the monogame-extended atlas. Aseprite `frameTags` become animations: each tag's source-frame run maps to a `<slug>-<tag>-NN` region run (matching `SpriteAnimationDef`/`DefineFrames`); untagged frames map to `<slug>-frame-NN`. Weapon anchors are **split**: the bat-side grip in frame-local pixels comes from a per-frame Aseprite `handle` slice pivot (`bounds.origin + pivot`, printed by the converter as `MeleeWeaponDef.CarryHandleOffset`/`SwingHandleOffsets` paste targets), the actor-side hand stays hand-tuned (`CarryHandAnchor`/`SwingHandAnchors`), and each anchor is the one-line formula `hand - (handleOffset - FrameCenter) * Scale` where `Scale` is the weapon's render scale relative to the actor (default `1`; the bat uses `0.5`). The hand anchors are absolute actor-unit offsets from `Position` (not relative to each other — the carry to swing sweep is authored explicitly). The overlay positions the region center at `Position + anchor * actorScale` and scales the texture by `actorScale * Scale` — the weapon scale cancels out of the grip-on-hand invariant for any value. Debug draw overlays the weapon region center / anchor (orange, with name label) and the bat grip point (green) — the green grip lands on the actor hand exactly when the anchor formula holds, and is flagged with a `Debug.WriteLine` warning when it falls outside the actor's sprite frame (the invariant that the bat-side grip must sit on the actor-side attach point). A bat redraw needs only re-export + re-paste the offsets — no actor-hand retune.
...
* **Skill File Updates**: As part of any proposed change, consider whether updating the skill files under `.kilo/skills/` would improve future skill runs — this applies whether the changes were made with a skill or without one. When a change alters workflows, tools, file paths, conventions, or code that a skill describes or references, update the relevant `SKILL.md` in the same change (alongside the same `LEARNING.md`/`MANUAL_TESTING.md`/`AGENTS.md` upkeep) so skills stay current and the next run follows the new reality instead of stale instructions.
...
* No sprite in the codebase currently frame-steps: player, enemy, and oil drum are time-driven via `Sprite.Update(gameTime)`, and the weapon overlay draws regions directly by name (`MeleeWeaponDef.ResolveWeaponRegionName`) instead of frame-stepping an `AnimatedSprite`. Reintroduce the manual frame-step pattern only if a future effect needs it, and pair every `SetFrame` with the region sync above.
