...
public abstract class LevelDirectorCore<TEnemy>
    where TEnemy : CombatActorBase, IPickupDropper
{
    ...
    private readonly List<TEnemy> _activeEnemies = [];
    ...
    public IReadOnlyList<TEnemy> ActiveEnemies => _activeEnemies;
    ...
    {
        var waves = Level.WaveDefs;

        if (_currentWaveIndex >= waves.Count)
        {
            if (_player.Position.X >= Level.EndTriggerX)
    ...
    {
        var wave = Level.WaveDefs[_currentWaveIndex];
        ...
        {
            var enemy = EnemyPool.Rent(def.Type, Vector2.Zero, _player);

            // Spawn sizing uses the enemy's own dimensions, not hard-coded art constants.
            float halfW = enemy.Width * 0.5f;
            float halfH = enemy.Height * 0.5f;
            enemy.Position = ComputeSpawnPosition(def.Side, def.Vertical, cameraLeftEdge, gameWidth, walkableTop, walkableBottom, halfW, halfH);
    ...
    private (float CameraLeftEdge, float GameWidth, float ViewportHeight, float WalkableTop, float WalkableBottom) GetSpawnContext()
    {
        ...
        var walkableBounds = Level.MovementBounds;
    ...
    public void DrawDebug(DebugDrawContext context)
    {
        ...
        context.SpriteBatch.DrawLine(Level.EndTriggerX, 0, Level.EndTriggerX, viewportHeight, Color.Orange * 0.4f, 2f);

        float levelRight = Level.MovementBounds.Right;
        context.SpriteBatch.DrawLine(0, Level.WalkableTopY, levelRight, Level.WalkableTopY, Color.Lime * 0.5f, 2f);
